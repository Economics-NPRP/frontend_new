...
type IFunctionSignature = (emissionId: number, auctionId: string, firmId: string, quantity: number, notes: string) => Promise<ServerData<{}>>;

export const acceptPermits: IFunctionSignature = cache(async (emissionId, auctionId, firmId, quantity, notes) => {
  ...
  const response = await fetch(await internalUrl(`/api/proxy/v1/permits/approve`), querySettings);
...
export const rejectPermits: IFunctionSignature = cache(async (emissionId, auctionId, firmId, quantity, notes) => {
  const t = await getTranslations();
  ...
  const response = await fetch(await internalUrl(`/api/proxy/v1/permits/reject`), querySettings);
  const rawData = camelCase(await response.json(), 5) as ServerData<unknown>;
