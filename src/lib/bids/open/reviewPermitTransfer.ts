...
type IFunctionSignature = (emissionId: number, firmId: string, quantity: number, notes: string) => Promise<ServerData<{}>>;

export const acceptPermitTransfer: IFunctionSignature = cache(async (emissionId, firmId, quantity, notes) => {
  ...
  const response = await fetch(await internalUrl(`/api/proxy/v1/users/firms/permits/transfer/request`), querySettings);
