...
export const startCycle: IFunctionSignature = cache(async (cycleId) => {
  ...
  const response = await fetch(
    await internalUrl(`/api/proxy/v1/cycles/${cycleId}/start`),
    querySettings,
  );
  const rawData = camelCase(await response.json(), 5) as OffsetPaginatedData<unknown>;

  //	If theres an issue, return the default data with errors
  if (response.status === 400) return getDefaultData(t('lib.cycles.start.invalidStatus'));
