...
export interface IGetPaginatedAuctionResultsOptions extends IOffsetPagination {
	auctionId: string;
...
type IFunctionSignature = (
	options: IGetPaginatedAuctionResultsOptions,
) => Promise<OffsetPaginatedData<IAuctionResultsData>>;

export const getPaginatedAuctionResults: IFunctionSignature = cache(
	async ({ auctionId, page, perPage, sortBy, sortDirection }) => {
		...
		const queryUrl = new URL('/v1/results/s', process.env.NEXT_PUBLIC_BACKEND_URL);
...
export const preloadPaginatedAuctionResults: IFunctionSignature = async (options) => {
	void getPaginatedAuctionResults(options);
}; 