...
type IFunctionSignature = (uuid: string) => Promise<ServerData<ISubsectorData>>;
export const getSingleSubsector: IFunctionSignature = cache(async (uuid) => {
	...
	const queryUrl = new URL(`/v1/subsectors/${uuid}`, process.env.NEXT_PUBLIC_BACKEND_URL);
...
export const preloadSingleSubsector: IFunctionSignature = async (options) => {
	void getSingleSubsector(options);
};
